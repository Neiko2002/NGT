...
public:
    ...
    GraphOptimizer(int outgoing,
                   int incoming,
                   int nofqs,
                   int nofrs,
                   float baseAccuracyFrom,
                   float baseAccuracyTo,
                   float rateAccuracyFrom,
                   float rateAccuracyTo,
                   double gte,
                   double m,
                   bool unlog  // stderr log is disabled.
    ) {
        init();
        set(outgoing, incoming, nofqs, nofrs, baseAccuracyFrom, baseAccuracyTo, rateAccuracyFrom, rateAccuracyTo, gte, m);
        logDisabled = unlog;
    }
    ...
    static std::pair<size_t, double> searchMinimumQueryTime(NGT::Index& index, size_t prefetchOffset, int maxPrefetchSize, size_t seedID) {
        NGT::ObjectSpace& objectSpace = index.getObjectSpace();
        int step = 256;
        ...
        for (step = 256; step != 32; step /= 2) {
            ...
            for (int prefetchSize = prevPrefetchSize - step < 64 ? 64 : prevPrefetchSize - step; prefetchSize <= maxPrefetchSize;
                 prefetchSize += step) {
    ...
    void execute(const std::string inIndexPath, const std::string outIndexPath) {
        std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000 << " Mb"
                  << std::endl;
        ...
        std::cerr << "Optimizer::execute: Copy index to " << outIndexPath << std::endl;
        {
            auto index = NGT::Index(inIndexPath);
            index.save(outIndexPath);
            index.close();
        }
        std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
                  << " Mb after copy index" << std::endl;
        ...
        */

        {
            ...
            std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
                      << " Mb after opening index" << std::endl;
            ...
            if (numOfOutgoingEdges > 0 || numOfIncomingEdges > 0) {
                ...
                try {
                    std::cerr << "Optimizer::execute: Extract the graph data. memory usage: " << getCurrentRSS() / 1000000
                              << " Mb, peak memory usage: " << getPeakRSS() / 1000000 << " Mb" << std::endl;
                    ...
                    timer.stop();
                    std::cerr << "Optimizer::execute: Graph reconstruction time=" << timer.time
                              << " (sec). memory usage: " << getCurrentRSS() / 1000000
                              << " Mb, peak memory usage: " << getPeakRSS() / 1000000 << " Mb" << std::endl;
            ...
            if (shortcutReduction) {
                ...
                try {
                    ...
                    timer.stop();
                    std::cerr << "Optimizer::execute: Path adjustment time=" << timer.time
                              << " (sec). memory usage: " << getCurrentRSS() / 1000000
                              << " Mb, peak memory usage: " << getPeakRSS() / 1000000 << " Mb" << std::endl;
                    graphIndex.saveGraph(outIndexPath);
                } catch (NGT::Exception& err) {
    ...
    void optimizeSearchParameters(const std::string outIndexPath) {
        if (searchParameterOptimization) {
            ...
            std::cerr << "memory usage: " << getCurrentRSS() / 1000000 << " Mb, peak memory usage: " << getPeakRSS() / 1000000
                      << " Mb after optimizing search parameters" << std::endl;
        }
        ...
        if (searchParameterOptimization || prefetchParameterOptimization || accuracyTableGeneration) {
            ...
            if (prefetchParameterOptimization) {
                ...
                std::cerr << "memory usage: " << getCurrentRSS() / 1000000 << " Mb, peak memory usage: " << getPeakRSS() / 1000000
                          << " Mb after optimizing prefetch parameters" << std::endl;
            }
            ...
            if (accuracyTableGeneration) {
                ...
                std::cerr << "memory usage: " << getCurrentRSS() / 1000000 << " Mb, peak memory usage: " << getPeakRSS() / 1000000
                          << " Mb after generating the accuracy table" << std::endl;
            }
    ...
    optimizeNumberOfEdgesForANNG(
        NGT::Optimizer& optimizer, std::vector<std::vector<float>>& queries, size_t nOfResults, float targetAccuracy, size_t maxNoOfEdges) {
    ...
    static std::pair<size_t, float> optimizeNumberOfEdgesForANNG(NGT::Index& index, ANNGEdgeOptimizationParameter& parameter) {
        ...
        if (transition.size() < 2) {
            std::stringstream msg;
            msg << "Optimizer::optimizeNumberOfEdgesForANNG: Cannot optimize the number of edges. Too small object set. # of objects="
        ...
        size_t estimatedEdge =
            std::get<0>(transition[0].second) + edgeRate * (log2(parameter.targetNoOfObjects) - log2(transition[0].first));
        float estimatedAccuracy =
            std::get<1>(transition[0].second) + accuracyRate * (log2(parameter.targetNoOfObjects) - log2(transition[0].first));
    ...
    std::pair<size_t, float> optimizeNumberOfEdgesForANNG(const std::string indexPath,
    ...
    void setExtension(float baseAccuracyFrom, float baseAccuracyTo, float rateAccuracyFrom, float rateAccuracyTo, double gte, double m) {
    ...
    void setProcessingModes(bool shortcut = true, bool searchParameter = true, bool prefetchParameter = true, bool accuracyTable = true) {
