...
public:
        ...
        for (size_t id = 1; id < graphIndex.repository.size(); id++) {
            ...
            try {
                ...
                if (graph.back().size() != graph.back().capacity()) {
                    std::cerr << "GraphReconstructor::extractGraph: Warning! The graph size must be the same as the capacity. " << id
                              << std::endl;
                }
    ...
    #if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                        outGraph.addEdge(
                            id, node.at(i, outGraph.repository.allocator).id, node.at(i, outGraph.repository.allocator).distance, true);
    #else
    ...
    static bool edgeComp(NGT::ObjectDistance a, NGT::ObjectDistance b) { return a.id < b.id; }
    ...
    static void insert(NGT::GraphNode& node, size_t edgeID, NGT::Distance edgeDistance, NGT::GraphIndex& graph) {
        ...
        GraphNode::iterator ni =
            std::lower_bound(node.begin(graph.repository.allocator), node.end(graph.repository.allocator), edge, edgeComp);
    ...
    static bool hasEdge(NGT::GraphIndex& graph, size_t srcNodeID, size_t dstNodeID) {
        ...
        GraphNode::iterator ni = std::lower_bound(
            srcNode.begin(graph.repository.allocator), srcNode.end(graph.repository.allocator), ObjectDistance(dstNodeID, 0.0), edgeComp);
            ...
            try {
                ...
                for (size_t sni = 0; sni < srcNode.size(); ++sni) {
                    ...
                    neighbors[srcNode.at(sni, outGraph.repository.allocator).id] =
                        std::pair<size_t, double>(sni, srcNode.at(sni, outGraph.repository.allocator).distance);
#else
...
#endif
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                            candidates.push_back(std::pair<int, std::pair<uint32_t, uint32_t>>(
                                (*dstNode).second.first,
                                std::pair<uint32_t, uint32_t>(srcNode.at(sni, outGraph.repository.allocator).id, dstNodeID)));
#else
                            candidates.push_back({static_cast<uint32_t>((*dstNode).second.first),
                                                  std::pair<uint32_t, uint32_t>(srcNode[sni].id, dstNodeID)});
#endif
                ...
                try {
                    ...
                    if (rank >= srcNode.size()) {
                        if (!removeCandidates[idx].empty() && minNoOfEdges == 0) {
                            std::cerr << "Something wrong! ID=" << id << " # of remaining candidates=" << removeCandidates[idx].size()
                                      << std::endl;
                            abort();
                        }
#if !defined(NGT_SHARED_MEMORY_ALLOCATOR)
                    ...
                    if (removeCandidates[idx].size() > 0 && ((*outGraph.getNode(id)).size() + srcNode.size() - rank) > minNoOfEdges) {
                        ...
                        while (!removeCandidates[idx].empty() &&
                               (removeCandidates[idx].back().second == srcNode.at(rank, outGraph.repository.allocator).id)) {
#else
                    ...
                    insert(outSrcNode,
                           ...
                           srcNode.at(rank, outGraph.repository.allocator).distance,
                           outGraph);
#else
    ...
    static void reconstructGraph(std::vector<NGT::ObjectDistances>& graph,
                                 NGT::GraphIndex& outGraph,
                ...
                if (originalEdgeSize == 0) {
                    ...
                    if (n.size() < originalEdgeSize) {
                        ...
                        if (warningCount <= warningLimit) {
                            std::cerr << "GraphReconstructor: Warning. The edges are too few. " << n.size() << ":" << originalEdgeSize
                                      << " for " << id << std::endl;
                        }
                        if (warningCount == warningLimit) {
        ...
        for (size_t id = 1; id < outGraph.repository.size(); id++) {
            try {
                ...
                if (id % 100000 == 0) {
                    std::cerr << "Processed " << id << " nodes. memory usage: " << getCurrentRSS() / 1000000
        ...
        std::cerr << "Reconstruction time=" << originalEdgeTimer.time << ":" << reverseEdgeTimer.time << ":" << normalizeEdgeTimer.time
                  << std::endl;
    ...
    static void reconstructANNGFromANNG(std::vector<NGT::ObjectDistances>& graph, NGT::Index& index, size_t edgeSize) {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    ...
    static void refineANNG(NGT::Index& index,
                           bool unlog,
                           float epsilon = 0.1,
                           float accuracy = 0.0,
                           int noOfEdges = 0,
                           int exploreEdgeSize = INT_MIN,
    ...
    static void refineANNG(NGT::Index& index,
        ...
        size_t noOfSearchedEdges =
            noOfEdges < 0 ? -noOfEdges : (noOfEdges > prop.edgeSizeForCreation ? noOfEdges : prop.edgeSizeForCreation);
