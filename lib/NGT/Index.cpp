...
static void insertMultipleSearchResults(GraphIndex &neighborhoodGraph, CreateIndexThreadPool::OutputJobQueue &output, size_t dataSize) {
  ...
  if (neighborhoodGraph.NeighborhoodGraph::property.graphType == NeighborhoodGraph::GraphTypeANNG ||
    ...
    // This processing occupies about 30% of total indexing time when batch size
    // is 200. Only initial batch objects should be connected for each other.
  ...
  for (size_t i = 0; i < dataSize; i++) {
    ...
    if (static_cast<int>(gr.id) > neighborhoodGraph.NeighborhoodGraph::property.edgeSizeForCreation &&
      ...
      cerr << "createIndex: Warning. The specified number of edges could not "
              "be acquired, because the pruned parameter [-S] might be set."
           << endl;
