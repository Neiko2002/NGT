...
int main(int argc, char** argv) {
    ...
    // // std::vector<float> exploration_coefficients = {0.015f, 0.016f, 0.017f, 0.018f, 0.019f, 0.02f, 0.021f, 0.022f, 0.023f, 0.024f,
    // 0.025f, 0.026f, 0.027f, 0.028f, 0.029f, 0.03f, 0.05f}; K = 100; repeat_test = 20;
    ...
    std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
              << " Mb after loading index" << std::endl;
    ...
    std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
              << " Mb after loading query data" << std::endl;
    ...
    for (float exploration_coefficient : exploration_coefficients) {
        ...
        auto time_us_per_query =
            (std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_begin).count()) / (query_num * repeat_test);
        auto recall = 1.0f * correct / repeat_test / (query_num * K);
        std::cout << string_format(
            "exploration_coefficient %.3f, recall %.4f, time_us_per_query %8d \n", exploration_coefficient, recall, time_us_per_query);
        // if (recall > 1.0)
        //   break;
    }
