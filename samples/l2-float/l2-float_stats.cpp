...
static auto read_top_list(const char* fname, size_t& d_out, size_t& n_out) {
    ...
    if (ec != std::error_code{}) {
        std::cerr << "error when accessing top list file" << fname << " size is: " << file_size << " message: " << ec.message()
                  << std::endl;
        perror("");
        abort();
    }
...
static uint32_t compute_reachablity_count(std::vector<std::vector<uint32_t>>& graph) {
    ...
    for (size_t i = 1; i < graph_size; i++)
        if (checked_ids[i]) checked_node_count++;
    ...
    for (size_t entry_id = 1; entry_id < graph_size; entry_id++) {
        ...
        while (check.size() > 0 && best_reach_vertex_reach < graph_size - 1) {
            ...
            for (size_t c = 0; c < check.size() && best_reach_vertex_reach < graph_size - 1; c++) {
                ...
                if (neighbor_indizies.size() == 0) std::cout << "zero out-degree for vertex " << check_index << std::endl;
                    ...
                    if (checked_ids[neighbor_index] == false) {
                        ...
                        if (vertex_reach_index < graph_size) {
                            ...
                            if (neighbor_reach.reach_count > best_reach_vertex_reach) {
                                ...
                                if (neighbor_reach.reach_count == graph_size - 1) break;
        ...
        if (best_reach_vertex_reach == graph_size - 1) {
            ...
            for (size_t i = 1; i < graph_size; i++) reach_count += checked_ids[i];
...
static void compute_stats(const char* graph_file, const uint32_t feature_dims, const char* top_list_file) {
    ...
    uint64_t oversize_neighborhood = 0;
    {
        ...
        std::cout << "Load TopList from file" << top_list_file << " with " << top_list_count << " elements and k=" << top_list_dims
                  << std::endl;
    ...
    for (uint32_t n = 1; n < graph_size; n++) {
        ...
        if (in_degree < min_in) min_in = in_degree;
        if (max_in < in_degree) max_in = in_degree;
        if (in_degree == 0) source_nodes++;
    }
    std::printf(
        "GQ %.4f, avg degree %.1f, min_out %d, max_out %d, min_in %d, max_in %d, source vertices %d, vertex count %zd, oversize "
        "neighborhood %zd\n",
...
int main(int argc, char** argv) {
    ...
    // const auto indexPath     = R"(e:/Data/Feature/SIFT1M/NGT/anng100_onng10_120_default)";  // GQ 0.973031, PNR 0.351096, avg
    // degree 49.3349, min_out 4,  max_out 626, min_in 18, max_in 143, zero in nodes 0 const auto indexPath     =
    // R"(e:/Data/Feature/SIFT1M/NGT/anng500-onng50_150_default)";        // GQ 0.98296,  PNR 0.405321, avg degree 63.0182, min_out 15,
    // max_out 531, min_in 23, max_in 318, zero in nodes 0
