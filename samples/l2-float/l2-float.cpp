...
int createANNGIndex(const char* indexPath, const char* featurePath, const int threadNumber) {
    try {
        ...
        // property.edgeSizeForCreation = 230;                  // L=230 is ef_construction and limits the number of search result (ANNG
        // parameter) property.insertionRadiusCoefficient = 1.03;          // not in weaves (ANNG parameter) property.outgoingEdge = 30; //
        // out=30 (ONNG parameter) property.incomingEdge = 100;                         // in=100 (ONNG parameter)

        // // WEAVES ANNG on GloVe
        // property.edgeSizeLimitForCreation = 300;             // K=200 is nn and limits the number of neighbors (ANNG parameter)
        // property.edgeSizeForCreation = 300;                  // L=230 is ef_construction and limits the number of search result (ANNG
        // parameter) property.insertionRadiusCoefficient = 1.03;          // not in weaves (ANNG parameter) property.outgoingEdge = 20; //
        // out=30 (ONNG parameter) property.incomingEdge = 100;                         // in=100 (ONNG parameter)
        ...
        // property.edgeSizeLimitForCreation = 200;             // K=200 is nn and limits the number of neighbors (ANNG parameter)
        // property.edgeSizeForCreation = 200;                  // L=200 is ef_construction and limits the number of search result (ANNG
        // parameter) property.insertionRadiusCoefficient = 1.1;           // not in weaves but default (ANNG parameter)
        ...
        std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
                  << " Mb after creating ANNG index files" << std::endl;
        ...
        std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
                  << " Mb after copying features vectors to index" << std::endl;
        ...
        std::cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb, Max memory usage: " << getPeakRSS() / 1000000
                  << " Mb after creating ANNG" << std::endl;
...
int refineANNGIndex(const char* anngIndexPath, const char* rnngIndexPath, const int threadNumber) {
    try {
        ...
        // //    -> edgeSize = -1;	// dynamically prune the edges during search. -1 means following the index property inside the
        // anngIndexPath. 0 means using all edges. any other number limits number of neighbors checked during a search
...
int main(int argc, char** argv) {
    ...
    if (reconstructState == 1) return 1;
